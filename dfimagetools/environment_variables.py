...
class WindowsEnvironmentVariablesCollector:
    ...
    _WINDOWS_NT_KEY_VALUE_MAPPINGS = {"SystemRoot": "%SystemRoot%"}
    ...
    def Collect(self, registry):
        ...
        if registry_key:
            yield from self._CollectEnvironmentVariablesFromEnvironmentKey(registry_key)
        ...
        registry_key = registry.GetKeyByPath(self._WINDOWS_NT_CURRENTVERSION_KEY_PATH)
        if registry_key:
            yield from self._CollectEnvironmentVariablesWithMappings(
